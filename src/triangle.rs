...
// This one isn't that amazing in the end. Should try finding something else.
fn point_in_triangle(point: (u32, u32), triangle: &[cg::Vector2<u32>]) -> bool {
...
pub fn draw(triangle: &[cg::Vector2<u32>],
    ...
    for y in bb_min_y..bb_max_y {
        ...
        for point in line.filter(|p| point_in_triangle(*p, triangle)) {
            framebuffer.setxy(point.0, point.1, color);
        }
    }
}
